  ...
  <!-- BEGIN: Subheader -->
					...
					<div class="m-content">
						...
						<div class="m-portlet m-portlet--mobile">
							<div class="m-portlet__body">
								<form class="m-form m-form--fit m-form--label-align-right" method="post" action="{{route('post.update',$post->id)}}" enctype="multipart/form-data">
										<div class="m-portlet__body">
		{{csrf_field()}}
                                                                                      @if($errors->any())
    ...
    @endif
											<div class="form-group m-form__group">
												...
												<input type="text" class="form-control m-input" id="name" name="title" value="{{$post->title}}">
                                    ...
                                    <select class="form-control m-select2" id="post_cats" name="cats[]" multiple="multiple">
                                        ...
                                        <option value="{{$cat->id}}" @if(in_array($cat->id,$post_categories)) selected @endif>{{$cat->name}}</option>
                                        @endforeach
                                                                </select>
                                                                </div>
                                            	<div class="form-group m-form__group">
